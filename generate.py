...
class Fetcher:
    ...
    def fetch_json(self, url: str, max_age: float | None = None) -> dict[str, Any]:
    ...
    def json_cache_stale(self, url: str, max_age: float) -> bool:
        path = self.api_cache / f"{self._key(url)}.json.gz"
        if not path.exists():
            return True
...
def discover_library_documents(fetcher: Fetcher, library: Library) -> list[Document]:
    ...
    if actual_code and actual_code.casefold() != library.code.casefold():
        raise ValueError(
...
def normalize_markdown(markdown: str) -> str:
    ...
    # The source emits empty heading-anchor markers like <span id="..."></span>
    # and <a id="..."></a>; they carry no text and only add noise for LLM
    # consumption. Over 99% sit on their own line, so remove those lines; leave
    # inline ones untouched.
    markdown = re.sub(r"(?m)^<span id=\"[^\"]*\"></span>\n", "", markdown)
    markdown = re.sub(r"(?m)^<a id=\"[^\"]*\"></a>\n", "", markdown)
...
def extract_document(
    ...
    # The site canonicalizes LibraryCode to lowercase (e.g. "modelark") while
    # URLs keep their original casing ("ModelArk"), so compare case-insensitively.
    returned_library = str(
        cur_doc.get("LibraryCode") or source.get("curLibCode") or ""
    )
    if (
        returned_code != document.code
        or returned_library.casefold() != document.library_code.casefold()
    ):
        raise ValueError(
...
def main() -> int:
    ...
    documents_to_extract = [
        ...
        or fetcher.json_cache_stale(document_api_url(document), args.max_age)
    ]
